...
这部分已经用真实问题验证跑通过：问"阿里巴巴和英伟达最近有什么相关的AI新闻？"，
模型自己规划了 4 轮工具调用（bm25_search → vector_search → graph_search("阿里巴巴")
→ graph_search("英伟达")），第 4 轮才给出综合回答，过程中自己判断"这个问题该换个
方式再查一次"，这就是"多轮迭代式 Agent"和"查一次就返回"的关键区别。课堂上会
照着这个思路重新手写一遍。整个多轮循环、工具分发、结果拼装的框架代码是完整的，
唯一缺的就是"发请求给模型、拿到它的决策"这一步。
"""
...
SYSTEM_PROMPT = (
    ...
    "知识库内容是 AI News（https://ai-news.tayoru-kun.com/）最近几天的 AI 行业新闻，"
    "按模型/工具/协议/平台/研究五个分类整理。"
...
#
# 实测效果：问"阿里巴巴和英伟达最近有什么相关的AI新闻？"，
# 第 1 轮调用 bm25_search("阿里巴巴 英伟达") 查到几条阿里巴巴的新闻，
# 第 2 轮调用 vector_search 补充语义相关的内容，第 3 轮调用 graph_search("阿里巴巴")
# 查到它和 Qwen3.8-Max、Arena.AI 的关系，第 4 轮调用 graph_search("英伟达") 却没查到
# 结果（说明图谱里这个实体名没有精确匹配上）——模型没有因此卡住，而是如实在最终
# 回答里指出"知识库里没有两家公司直接相关的新闻"，同时分别列出了两家公司各自的
# 最新动态，并注明结论来自关键词/语义检索。这也说明了 Agentic 查询的一个特点：
...
if __name__ == "__main__":
    q = sys.argv[1] if len(sys.argv) > 1 else "阿里巴巴和英伟达最近有什么相关的AI新闻？"
    query_agentic(q)
