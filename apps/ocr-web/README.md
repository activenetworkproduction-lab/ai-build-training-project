...
| 步骤 | 文件 |
|---|---|
| ① 前端上传 | `apps/ocr-web/src/App.tsx` 的 `handleParse()` |
| ① 后端接收 | `apps/server/src/modules/ocr/ocr.controller.ts` |
| ②③④ 调用模型 | `apps/server/src/modules/ocr/ocr.service.ts` 的 `parseImage()` |
...
2. **服务端环境变量（适合部署）**：启动后端前设置 `GEMINI_API_KEY` 或
   ...
   ```powershell
   $env:GEMINI_API_KEY = "你的Key"; pnpm dev:server
   ```
...
pnpm dev:ocr      # 前端 http://localhost:5102
```
