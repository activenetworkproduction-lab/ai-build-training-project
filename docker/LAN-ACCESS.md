...
本机（`anc-tarounoMini.anc.lan`）可接入两个网络，两套 Docker 数据库基础设施在这两个网络上都已开放访问：

- 有线网络（`en0`）：`192.168.20.174` — **当前未接入线缆，此地址暂不可用**
- Wi-Fi 网络（`en1`）：`192.168.68.52` — 当前生效

**请根据你自己电脑连接的是哪个网络，选择对应的 IP**（用 `ipconfig`/`ifconfig` 看一下自己的网段，跟哪个前缀一致就用哪个 IP；两个网络互不相通，用错网段会连不上）。目前只有 Wi-Fi 网络在线，有线网口恢复连接后该地址会重新可用，具体以维护者最新通知为准。
